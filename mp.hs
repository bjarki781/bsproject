...
-- all elements in D_4
actions :: [Perm -> Perm]
actions = [id, actionR, actionR . actionR, actionR . actionR . actionR, 
           actionH, actionV, actionR . actionH, actionR . actionV]
  where 
...
trm_cfl_systems :: [([Equivalence], Maybe System)]
trm_cfl_systems = 
  filter (isJust . snd)
...
main :: IO ()
main = do
    let to_print = map (\(e, d) -> (e, fromJust d)) $ ([],Just []): trm_cfl_systems 
    let findSystemClustersDict sys = [((p, q), c) | p <- dom sys, q <- dom sys, c <- findClusters p q, not $ null c]

    mapM_ (\(e, d) -> print (e, d, dom d, findSystemClustersDict d)) to_print
