...
\begin{lemma}
    Let $R$ be a rewrite system. If $\pi \patternrule \rho$ is in $R$ and 
    \[
        \sum\nolimits_{\img(R)}(u\pi v) < \sum\nolimits_{\img(R)}(u \rho v) \tag{$\ast$}
        \label{increasingimage}
    \]
    then $\sum$ parameterized by the image of $R$ is an increasing statistic
    with respect to $R$.
\end{lemma}
If \eqref{increasingimage} applies to a system then we see that a set of it's
...
\begin{lemma}
    ...
    \]
    for each $\pi \patternrule \rho \in R$ then $R$ fulfills condition
    \eqref{increasingimage}. 
\end{lemma}
...
\begin{proof}
    ...
    \[
        ...
        \label{increasingimage}
    \]
    We have to make sure that clusters between images and domains of seperate
    systems are all shallow.
\end{proof}
...
We shall prove that each algorithm doesn't ruin the termination of the system
under the statistic $\img(R)$, original $R$. With image.

\begin{proof}
All rules are on the form
domdom imgdom domimg. We want to show that a rule added with the algorithm does
not ruin any img appearences it might cluster with and that rule also obeys
\eqref{increasingimage}. For the original image.
\end{proof}
